...
In the SM this mechanism is called Brout-Englert-Higgs (BEH) mechanism, proposed by Brout-Englert-Higgs in 1964. It introduces a complex scalar field $\phi$, defined as a doublet in $SU(2)_{L}$ space and with non-zero hypercharge under $U(1)_Y$ and represented as singlet in $SU(3)_{C}$ color space:
\begin{equation}
\label{eq:higgsphi}
\phi = \begin{pmatrix}
\phi^+\\
...
To conclude, while the boson mass term is determined by a gauge principle, the fermionic couplings with the Higgs field are not necessary. At the same time, the Yukawa coupling constants and the fermion masses are free parameter of the theory while, on the other hand, the boson masses are fixed by couplings of the gauge interactions and the vacuum expectation value of the Higgs field.
...
It was found a significant deficit of neutrinos coming from the sun with respect to the solar-model calculations of the neutrino flux from decay of $B^8$\footnote{For the experiment at the Brookhaven National Laboratory, the calculations of the flux of solar neutrino coming from boron-8 decay were used: ${}^{8}B\rightarrow {}^{8}Be + e^{+} + \nu_e$. For a simple scheme and explanation see webpage~\cite{webpage_boron}.} inside the sun. This was the first hint of the so-called \emph{solar neutrino problem}.\\
...
\subsection{Summary}\label{sec:summaryC1}
