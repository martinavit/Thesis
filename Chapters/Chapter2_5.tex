...
from the point where the interaction occurs, particles enter the
...
produce in the muon-system signal hits and their trajectory is finally defined. This simple
...
is presented in Section~\ref{sec:trackvertex} and the particle-flow
...
leptons that originate from the primary
...
particles that have a lifetime ``long'' enough to travel from a few mm up
...
particle tracks is a crucial element for accurate measurement of
...
position of a hit is determined based on the geometry of the single-pixel or strip while taking into consideration the Lorentz drift due
...
sophisticated reconstruction that uses the information about the
...
Additional input for the track reconstruction is the identification
...
trajectory is extrapolated to the subsequent layer. In the end, tracks
that do not fulfill goodness-of-fit requirements are rejected. Tracks
that are ``easy'' to reconstruct, e.g. from particles with large \pt
...
with these tracks are taken out, making the subsequent iterations less
...
Vertices are reconstructed using high-quality tracks which are
...
number of tracks associated with the vertex and it is estimated to
...
resolutions vary between 10 and 100
...
$p^2_T$ sum of all the physics objects associated with it. The other vertices are assigned as pileup
vertices.
...
to make together colorless objects. The collection of these objects is named 
...
is known as the anti-$k_T$ algorithm (with distance parameter of the cone size
...
jet which comes from different types of PF candidates~\cite{CMS-PAS-JME-16-003}.
...
corrections are obtained from the ratio between the \pt of a
reconstructed jet and its corresponding generated jet and they are
measured as a function of \pt and $\eta$ and applied to data and
...
The identification of jets that originate from b quarks, called
...
The b jets contain b hadron that has a lifetime of the order of 1.5
...
means that their decay products have larger momentum and a larger number
of tracks related to the jet. These signatures are exploited by the
algorithms building variables to separate b jets from
light jets. The results of the b tagging are discriminator values, see Figure~\ref{fig:taggingperformance}; large discriminator values coincide with a higher probability for
...
algorithm based on a fully-connected neural network with a fixed set
...
muon system identifies muon objects over a wide acceptance with high efficiency. High purity in muons is obtained
...
\item \textbf{tracker muon}. These tracks are formed
  ...
  least one muon system segment matches that track then the muon track is defined as \emph{tracker
  muon track}. 
\item \textbf{global muon}. These tracks are built
  ...
  comparing tracks parameters that are propagated onto a shared
...
For muons that are not global muons and
are matched with the innermost muon station only,
...
compatibility} is an important parameter that returns values between 0
...
\item \emph{Loose muon identification, (Loose ID)}. A loose muon is either a
  ...
  very high selection efficiency and a rather low misidentification rate
...
tracks that have at least 80\% of valid hits in the inner tracker. 
...
\item \emph{Tight muon identification, (Tight ID)}. A tight muon must
  ...
  the number of muon stations that have to match with the inner
...
\caption{Tag-and-probe efficiency for muon identification in 2016 data
  (circle) and simulation (squared). On the left, Loose ID efficiencies as a function of $\eta$. 
On the right, Medium ID efficiencies as a function of $\eta$. The
...
the CMS capabilities in identifying and triggering on
...
average lifetime of 57 mm ($c\tau$), which corresponds to an average
...
valid tracker hit fraction requirement are shown. On the right, the efficiency of the tracker-muon only and
    global-muon only requirements are compared with their logical OR
...
selection only increases the overall efficiency by a few percent.\\
...
particle is created in the proximity of the beam
...
\item \textbf{Displaced Global Muon reconstruction.} Designed for displaced in-time muons produced within the inner-tracker volume, with the
  ...
  not use any constraints to the interaction point neither in the pattern recognition nor in the track fit.
\end{itemize}
...
function of \pt is shown. Both the efficiencies for ECAL-based
...
(which is usually smaller than a hadronic shower), on the ratio between the
...
Another possibility is a simple set of criteria that have to be
...
As for the muon case (refer to Section~\ref{sec:muoniso}), for electrons
...
reconstructed tracks is shown. It is estimated as a function of the flight
...
ones that are worth introducing at this stage of this dissertation. 
...
and~\ref{Chapter6}.\\
There are a number of variables that are referred specifically to as light
...
carry momentum energy which has to be estimated, or else it would
...
described in detail the PF algorithm with its input and output such as
leptons. \\
...
the end of this Chapter~\ref{Chapter2_5}; each of the variables will be found later on
in the text.
