...
In this limit, the mass matrix give rise to 3 Dirac neutrinos $\Psi = (\nu_L,\:\bar{\nu_R})$ with mass $m_\nu = m_D$. To obtain the observed neutrino masses then it is needed that coupling is $F_{\alpha I} \sim 10^{-12}$ which is much smaller than the SM Yukawa couplings.
...
Looking now at the Feynman diagram of the first term of Eq.~\ref{eq:dirac} a RH particle disappears at a point and a LH particle appears. Therefore weak (chiral) charge is not conserved, but the lepton number, however, is conserved, as we started with a neutrino (not an anti-neutrino) and ended up with a neutrino $\rightarrow$ \textbf{Dirac conserves the lepton number}.

Same considerations can be made for the Eq.~\ref{eq:majorana}, the weak charge is not conserved and neither the lepton number. We started with zero neutrinos and ended up with two neutrinos. $\rightarrow$ \textbf{Majorana violates the lepton number}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{5mm} 
\section{Theoretical and experimental constraints}
...
Considering the different MeV-TeV mass ranges and taking into account
...
Quite often the HNL bounds are shown in the 2D plane $mixing, \: V_{\alpha N} \longleftrightarrow
...
\begin{itemize}
\item PS191 (CERN, PS Beam 1983)~\cite{BERNARDI1988332}: the PS191
  experiment was specifically design to look for neutrino decays in
  low energy neutrino beam. It was a detector composed of a 12m long
  ...
  with electrons~\cite{Vannucci:1985v}.
\item CHARM (CERN, SPS beam 1985)~\cite{DORENBOSCH1986473}: A search
...
\item Belle (KEK, asymmetric-energy $e^+e^-$ collider, 2012)~\cite{Liventsev_2013}: Belle collaboration
...
\item DELPHI (CERN, lepton collider LEP, 1997)~\cite{Abreu:1996pa}:
  the most stringent limits between 1 GeV and 10GeV are still the ones
  been put by the DELPHI Collaboration. HNL search have been performed
  ...
  search, \hnl was looked for checking detectable secondary vertices or calorimeter clusters. 
 Upper limits were set for the branching ratio $BR, \: Z_0\rightarrow \hnl$ of
 about $1.3 \times 10^-6$ at 96\% C.L. for \hnl masses between 3.5 and
 50 GeV. 
\item CMS and ATLAS (CERN, LHC pp beam 2019): there have been several searches for HNLs in both CMS and ATLAS.
...
CMS searches with three charged leptons in the final state using the leptonic \PW decay are going to extensivly discussed in this
elaborate.
\end{itemize}
...
\begin{itemize}
\item Searches for Charged Lepton Flavor Violation. If we consider an HNL with a
  mass close to EW scale and with large off-diagonal Yukawa couplings,
  then this \hnl can lead to LFV in decays of charge leptons. Thus,
  testing LFV in multilepton searches could be an inderect way to
  probe the existance of HNL. Searches for such processes have
...
\item Cosmological constraints on light neutrino masses. The Planck
...
\end{equation}
\item BBN constraints. Observing
  Figs. ~\ref{fig:HNL_bc6_pbc_2},~\ref{fig:HNL_bc7_pbc_2}
...
\item \emph{Seesaw} limit. Below the line of the seesaw limit, the
