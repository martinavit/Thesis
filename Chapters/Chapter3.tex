...
\mathcal{L} = \mathcal{L}_{SM} + \mathcal{L}_{DS} + \sum F_{I \alpha} (\bar{L}_{\alpha}  \cdot \widetilde \Phi)N_{I} + h.c.
\end{equation}
...
If $\mathcal{N} = 3$, there is a right-chiral counterpart for each $\nu_{SM}$, see fig.~\ref{fig:c3sm_extension}. The fermion $N_I$ can have mass $M_I$, which is independent  of the value of $F_{I \alpha}$.
\begin{figure}[t!]
  \centering
...
$F_{I \alpha}$. The experimental sensitivity is expressed in terms of
the coupling $|V_{I \alpha}|^2$ ($= |F_{I \alpha}|^2$) as a function
...
zero; although it does not translate into a valid concrete model, this latter consideration it is very useful to evaluate the experimental sensitivity on the single $|V_{I \alpha}|^2$ without involving any model dependent hierarchy between the different flavor mixings. 
%\begin{figure}[h]
%  \centering
...
\label{eq:fullSMLag}
 \mathcal{L} = \mathcal{L}_{SM}+ i \bar N_I \partial_\mu \gamma^\mu N_I -
  \left(F_{I \alpha} \,\bar L_\alpha N_Is \tilde \phi 
...
\end{pmatrix}
\end{equation}
with the matrix elements defined as: $m_D = 3 \times  \mathcal{N}$ Dirac mass matrix, $(m_D)_{I \alpha} = F_{I \alpha}v, \; v = \sqrt{2}\langle \Phi \rangle$ and $M_I$ is $\mathcal{N} \times \mathcal{N}$ matrix of Majorana masses.
...
\begin{equation}
\label{eq:v2}
 |V_{I \alpha} |^{2}\equiv \frac{v^{2}|F_{I \alpha}|^2}{M^{2}_{I}} \ll 1
\end{equation}
...
The lifetime of a HNL is strongly dependent on $M_{N_I}$ and $|V_{I \alpha}|^2$,
...
c\:\tau_{N_{I}} \propto\mathrm{M_{N_I}^{-5}|V_{I \alpha}|^{-2}}
\end{equation}
...
neutrinos thanks to the tiny, but nonzero mixing $V_{I \alpha I}$, Eq.~\ref{eq:v2}.
...
\begin{figure}[h]
  ...
  \includegraphics[width=.99\textwidth]{Figures/c3/marcoscheme.png}
    \caption{A schematic overview from~\cite{DREWES2017250} of the allowed range of values for
