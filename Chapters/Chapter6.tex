...
(Fig.~\ref{fig:llfeatures} (top-left)), invariant mass smaller than
\mhnl (Fig.~\ref{fig:llfeatures} (top-right)), and a small opening angle
(Fig.~\ref{fig:llfeatures} (bottom-left)).
In the absence of significant hadronic activity, \lone and \hnl are
typically separated by a large azimuthal angle
(Fig.~\ref{fig:llfeatures} (bottom-right)).
...
As it is seen in Fig.~\ref{fig:cutflow1}, the most effective
...
Sec.~\ref{sec:jetclustering}) after subtracting the \pt of the two leptons:
\begin{linenomath}
  \begin{equation}
    \label{eq:ptjet}
    \ptm = \ptj =
...
Fig.~\ref{fig:svdiagrams}), giving the signature of two displaced tracks
...
from ``almost prompt'' to very displaced in Fig.~\ref{fig:invmass_mumuskim}.
...
relatively fine binning in Fig.~\ref{fig:2017_detector} (left). Notice how the pixel detector layout has a clear structural influence on the yield, both in simulation and data.
...
In Fig.~\ref{fig:2017_detector} (right), it is shown the comparison of data to simulation for the full
...
In Fig.~\ref{fig:2dplots}, we extend these results by performing a
...
\begin{figure}[h]
\noindent
\makebox[\textwidth]{  \includegraphics[clip,trim=0 0 2cm 0,width=.40\textwidth]{{Figures/c6/systematics/2018/e_ele/M-6.0_V-0.0019949937_normalized}.pdf}
  \includegraphics[clip,trim=0 0 2cm 0,width=.40\textwidth]{{Figures/c6/systematics/2018/mu_muo/M-6.0_V-0.0019949937_normalized}.pdf}}
...
\begin{figure}[h!]
  ...
  \caption{Values of the \PKzS\ mass peak (left) and width (right) as
    ...
    \PKzS\ mass profile (\eg, see Fig.~\ref{fig:invmass_mumuskim}). \dani}
...
\begin{figure}[h!]
    ...
    \caption{\label{fig:datadriven_2displaced}
        ...
        \HNLtwo, \HNLsix, \HNLtwelve. Events in the overflow bin are included in the last bin.
    }
\end{figure}

The predictions and observations in each of the search regions are
shown in Fig.~\ref{fig:finalyields_majorana_datacards}.
...
described in Sec.~\ref{sec:reweighting} to emulate HNLs of same mass and
...
test statistics, using the bins of Figs.~\ref{fig:finalyields_majorana_datacards}.
...
\begin{figure}[!ht]
    ...
    \caption{\label{fig:limits_Maj}
        ...
        and the CMS~\cite{Sirunyan:2018xiv,Sirunyan:2018mtv} Collaborations are shown for reference.
    }
\end{figure}
...
In the Dirac HNL scenario, we exclude electron (muon) couplings of 1.0\ten{$-4$} (2.0\ten{$-5$}) and higher for an HNL mass of 2\GeV, and of 0.7\ten{$-7$} (2.5\ten{$-8$}) for an HNL mass of 12\GeV.
...
CMS~\cite{Sirunyan:2018xiv,Sirunyan:2018mtv} Collaborations.
For the case of electron couplings, the limits improve withs
respect to the DELPHI results for masses above about 3.5\GeV.


\section{Summary}
A search for heavy neutral leptons (HNLs) in final states
with three leptons is performed using data from the CMS experiment
corresponding to an integrated luminosity of 137\fbinv. The analysis uses
dedicated methods to identify displaced leptons associated with
HNL decays. Several novel methods were developed to obtain estimates
for the relevant background processes from control samples in data, representing one of the important
challenges in this type of search at the LHC. No significant deviation
from the standard model predictions is observed in data. The 95\% confidence
level are set on HNL masses and coupling strengths of
these particles to standard model neutrinos. These results represent
the most stringent limits to date on this type of processes in the
