...
missing. Furthermore, the theory presents no clear direction on the new physics scale.
This forces refining and widening of the experimental effort
...
\item \emph{small coupling constants} -- \eg HNLs, %RPV SUSY (
  R-parity violating SUSY, %scenario),
  etc;
...
prompt particles, thus integrating displaced
...
\item \emph{background}: additional sources of background have to be
  considered, such as noise % maybe 'electronic noise'? But this is an issue for all analyses in principle
...
These are just a few examples of the almost endless configurations that can
...
according to the efficiency observed in data, by using the per-event scale
...
must have \pt greater than 30 (32, 32)\GeV in 2016 (2017, 2018)
data set.
...
misidentified as signal electrons (\fr or FR).
\begin{table}[h!]
  \centering
{\small
...
\includegraphics[width=.40\textwidth]{Figures/c6/selection/l1l3_dPhi_gen.png}
  \caption{For the HNL scenario: HNL of mass
    ...
    (``from $\hnl\PW$'').
Top right, the invariant mass of \ltwo
    and \lthree 
    at generator level.
...
misidentified leptons while retaining almost full efficiency for the
...
leptons forming an SV, the kinematic properties and particle content of
...
cuts on the two leptons bias the background to have a small angle as
well. For this reason, all
background processes exhibit a peaking structure, randomly at $-1$ or
$+1$.\\

The quality of the secondary vertex ($\boldsymbol{ p_{SV}}$)
...
\item {\color{gray}$\PZ\PGg^{\ast}$:}  it includes DY events with
  additional prompt lepton(s) from a virtual photon conversion;
...
\begin{figure}[h!]
\centering
  \includegraphics[clip,trim=0.9cm 0.7cm 0.cm 0.7cm,width=.48\textwidth]{Figures/c6/selection/18/e_cutflow__0.pdf}
  \includegraphics[clip,trim=0.9cm 0.7cm 0.cm 0.7cm,width=.48\textwidth]{Figures/c6/selection/18/mu_cutflow__0.pdf}\\
...
\begin{figure}[h!]
\centering
   \includegraphics[clip,trim=0.9cm 0.7cm 0.7cm 0.7cm,width=.48\textwidth]{Figures/c6/selection/eff_e_cutflow_n_1__0.pdf}
  \includegraphics[clip,trim=0.9cm 0.7cm 0.7cm 0.7cm,width=.48\textwidth]{Figures/c6/selection/eff_mu_cutflow_n_1__0.pdf}
  \caption{
...
Since the background yields are extremely low in large displacement
bins for the high-
...
\end{wrapfigure} 
mass category, we opted for a lower number of bins, as
follows:
\begin{itemize}
...
\item \textbf{Photon conversions:} this background was extensively
 introduced in Sections~\ref{sec:c4photon} and~\ref{sec:bgk}.\\
  ...
  three leptons with an invariant mass close to the \PZ mass (Section~\ref{sec:conversion}).
...
lepton is therefore associated with the jet of $\pt>10$\GeV closest in
\DR, if $\DR<0.4$.
If two \fo leptons 
are either associated with the same jet or have $\DR<0.45$, they are considered double-fake
...
the \dfr dependence on the displacement of the SV of the \hnl decay. 
...
all the detailed motivations). To a good extent, no strong dependence on
\dxy is
observed, nor on the different origins of fake leptons. Thus, \sfr is
parametrized to only depend on \ptc and \abseta, while the
dependence on \dxy is ignored. 
Figure~\ref{fig:sfr_dxy} shows that \sfr values are fairly independent
of the lepton \dxy, which is used as a proxy for the displacement. 
...
\begin{figure}[h!]
  \centering
  \includegraphics[clip,trim=0.cm 0.cm 0.5cm 0.5cm,height=4.8cm]{Figures/c6/backgrounds/mc_closure/mu_DeltaPV_SV_2D_zomm__0.pdf}
  \includegraphics[clip,trim=0.cm 0.cm 0.5cm 0.5cm,height=4.8cm]{Figures/c6/backgrounds/mc_closure/mu_M_ll_l2_l3_zoom__0.pdf}
  \includegraphics[clip,trim=0.5cm 0.cm 0.5cm 0.5cm,height=4.8cm]{Figures/c6/backgrounds/mc_closure/mu_SR__final.pdf}
...
defined search region, to test the predictive power of the method.
...
\begin{figure}[h]
  \noindent
  \makebox[\textwidth]{
  \includegraphics[width=.71\textwidth]{Figures/c6/backgrounds/FR/closureTest/Data/SideBandPlusBjet/mu_cr.pdf}}\\
  \makebox[\textwidth]{\includegraphics[width=.71\textwidth]{Figures/c6/backgrounds/FR/closureTest/Data/SideBandPlusBjet/ele_cr.pdf}}\\
\noindent
  \makebox[\textwidth]{
...
\end{table}
 ...
 \begin{figure}[h!]
\centering
\includegraphics[height=4cm]{Figures/c6/backgrounds/FR/closureTest/Data/conversion/M-6_V-0p00202484567313_e_ele_datacard_combined_SR.pdf}\\
  \includegraphics[height=4cm]{Figures/c6/backgrounds/FR/closureTest/Data/conversion/M-6_V-0p00202484567313_e_ele_mass_datacard_combined_massl2l3.pdf}
  \includegraphics[height=4cm]{Figures/c6/backgrounds/FR/closureTest/Data/conversion/M-6_V-0p00202484567313_e_ele_disp_datacard_combined_displacement.pdf}
...
For prompt electrons, the SFs are obtained following the CMS \emph{electron-photon physics object group} prescriptions, using the same
...
from the $ J/\psi$~meson decay originate from an SV. The
...
\begin{align}
\vcenter{\hbox{\includegraphics[clip,trim=0cm 0cm 0.5cm 0cm, width=.48\textwidth]{Figures/c6/efficiencies/figs_Displacement.png}}}
&\qquad
\begin{aligned}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.48\textwidth]{Figures/paper/convDisplacement-crop.pdf}
    \includegraphics[width=0.48\textwidth]{Figures/paper/convM4l-crop.pdf} \\
    \includegraphics[width=0.48\textwidth]{Figures/paper/convPVSV4l-crop.pdf}
    \caption{\label{fig:convdispl}
        ...
        four (upper right, lower) leptons,
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.34\textwidth]{Figures/paper/ksinvmass_run2_fig_0.pdf}
    \includegraphics[width=0.34\textwidth]{Figures/paper/ksinvmass_run2_fig_1.pdf} \\
    \includegraphics[width=0.34\textwidth]{Figures/paper/ksinvmass_run2_fig_2.pdf}
    \includegraphics[width=0.34\textwidth]{Figures/paper/ksinvmass_run2_fig_3.pdf} \\
    \caption{\label{fig:invmass_mumuskim}
        The invariant mass distribution of the \PKzS candidates was reconstructed using the $\pi^{\pm}\pi^{\mp}$ tracks for various displacement regions. The fitted \PKzS candidate mass in data in each region is also shown. \luka}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.44\textwidth]{Figures/c6/efficiencies/2017E_detector}
    \includegraphics[width=0.44\textwidth]{Figures/paper/K_short_Displacement.pdf}
...
a few \%). %where
In some search regions, the uncertainties are
dominated by the signal modeling, because of the limited size of the
signal MC samples.
...
are used to assess the modeling of \displ electron reconstruction, and long-lived \PKzS 
...
in data that is not simulated in MC (see
...
\item 3 additional uncorrelated nuisances ($\mu\mu$, $e\mu$ and $ee$) to account for the differences in sources and statistics between the 3 channels in the \Dfr measurement region.
\end{itemize}
The 30$\%$ covers all the discrepancies we observe in our control region closure and 50$\%$ is motivated by the lack of statistics in the control regions for the closure test. 
The closure tests in simulation and data in each data-taking period
show no particular dependence on the different data-taking periods, as such we consider the 30$\%$ and 50$\%$ systematic uncertainties correlated overall data-taking period and lepton channels, while the additional 3 flavor systematics are taken uncorrelated among the channels and the years. ($\mu\mu$, $e\mu$ and $ee$. For 2016 17, 20, 40 $\%$, for 2017 14, 14, 33 $\%$ and for 2018 10, 11, 25 $\%$).
...
In these events, the three leptons form a mass peak in the proximity of the Z boson
...
The statistical uncertainty in the predicted fake lepton background
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.49\textwidth]{Figures/c6/Figure_006-a.pdf}
    \includegraphics[width=.49\textwidth]{Figures/c6/Figure_006-b.pdf}\\
   \includegraphics[width=.49\textwidth]{Figures/c6/Figure_007-a.pdf}
    \includegraphics[width=.49\textwidth]{Figures/c6/Figure_007-b.pdf}
...
\begin{figure}[h!]
    \centering
     \includegraphics[width=.98\textwidth]{Figures/c6/Figure_008-a.pdf}\\
    \includegraphics[width=.98\textwidth]{Figures/c6/Figure_008-b.pdf}
...
The yields of the observed events in data show a good agreement with the
...
\begin{table}[h!]
    ...
    \input{tabs/bgk_sum_error_ele_combined.tex}
\end{table}
\vspace{10cm}
\begin{table}[h!]
    \footnotesize
    ...
    \input{tabs/bgk_sum_error_muon_combined.tex}
\end{table}
...
\begin{table}[h!]
    ...
    \caption{\label{tab:ele_yield}
        The number of predicted signal events in the \eex and \mmx final states.
        The results are presented for several benchmark signal hypotheses for Majorana HNL:
        \HNLtwo, \HNLsix, \HNLtwelve.
    }}
    \cmsTable{\input{tabs/2_signal_sum_error_ele_combined.tex}}\\
    \cmsTable{\input{tabs/2_signal_sum_error_muon_combined.tex}}
\end{table}
...
described in Section~\ref{sec:reweighting} to emulate HNLs of the same mass and
...
The simulation of Dirac HNL scenarios is obtained from
...
The analysis makes use of dedicated techniques to select
...
\centering
    \includegraphics[clip,trim=1.2cm 1cm 0.cm 0.5cm, width=.58\textwidth]{Figures/c6/results/signalStrength_mu_Dirac_muo_limits_atlas.pdf}
...
stringent limits on this type of process in the
...
The second comparison is made with the relatively recent ATLAS
...
I was the main analyzer responsible for designing the entire analysis from the beginning to the writing of the journal publication. I performed the measurement of the
...
I have actively participated in five \emph{LHC LLP Community}
