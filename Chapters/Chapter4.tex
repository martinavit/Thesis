...
simplistic way as follows: to give analyzers a sense of the type of
...
method~\cite{mc}. The simulated events
 which are grouped in specific
sample for each physics process are called MC samples.\\
...
which are located in proximity either of a detector layer or of a inert material.
...
This section is closely pertinent to the signal model used in
...
The HNL production is simulated via the charged current (CC) Drell-Yan
...
For signal simulation, they are produced HNLs of masses from 
$\mhnl = 1$\GeV to $\mhnl = 1.2$\TeV, which couple
...
$(\mhnl,\tau_{\mathrm T})$ and $L$ is the integrated luminosity,
the complete event weight is
\begin{linenomath}
  \begin{equation}
    w(t; \{\tau_i\}\to\tau_{\mathrm T}) ~=~
    \frac{\sigma_{\mathrm{T}}L}{N_{\mathrm{tot}}}\,W(t;\{\tau_i\}\to\tau_{\mathrm T})
     ~=~ \frac{\frac{\sigma_{\mathrm{T}}L}{\tau_{\mathrm{T}}}\exp{(-t/\tau_{\mathrm{T}})}}
         {\sum_i\frac{N_{i}}{\tau_i}\exp{(-t/\tau_i)}}.
...
displacement (\Deltwod) for an HNL scenario with
...
long-lived analysis presented in Chapter~\ref{Chapter6}, for detailed
...
\begin{figure}[h!]
  ...
  \caption{Distributions of the reconstructed dilepton mass \mtwol
    ...
    Sec.~\ref{sec:baselinesel} (right), for an HNL scenario with
    ...
    re-weighting of Eq.~\ref{eq:ctauReweighting}.}
...
values~\cite{Degrande:2016,heavyN}, and is the same for Dirac and
Majorana HNLs.
...
After the lesson learned with the emulation procedure described just
...
\texttt{SingleElectron}, \texttt{SingleMuon}, \texttt{DoubleEG} and 
\texttt{DoubleMuon} primary datasets; the choice was driven by the
multiple wanted triggers which are both single lepton trigger and
dilepton triggers.
...
The long-lived HNL analysis uses three sets (2016-2017-2018 datas
