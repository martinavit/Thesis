...
\noindent
\makebox[\textwidth]{
\includegraphics[width=0.25\textwidth]{Figures/c5/hnl_feyn.pdf}
\includegraphics[width=0.25\textwidth]{Figures/c5/hnl_feyn_2.pdf}
\includegraphics[width=0.25\textwidth]{Figures/c5/hnl_z_feyn.pdf}
\includegraphics[width=0.25\textwidth]{Figures/c5/hnl_z_feyn_2.pdf}}
...
information about the "primary dataset" see Chapter~\ref{Chapter4}.)
...
Chapter, were simulated with NLO precision in
perturbative QCD. Signal
...
\noindent
\makebox[\textwidth]{
\subfloat[]{\includegraphics[width=.34\textwidth]{Figures/c5/genPt/genPtm5.pdf}}
\subfloat[]{\includegraphics[width=.34\textwidth]{Figures/c5/genPt/genPtm60.pdf}}
\subfloat[]{\includegraphics[width=.34\textwidth]{Figures/c5/genPt/genPtm200.pdf}}}
...
"loose", "fakeable object" (FO) and "tight". The loose working point
...
within a cone of $\Delta \mathrm{R} = 0.05$ around a loose muon. The
FO working point is slightly tighter than the loose one, and is
...
tight working point should be independent
...
\caption{All the variables used in the table are introduced in
  Chapter~\ref{Chapter2} in Section~\ref{sec:reconstruction}.
Requirements to pass each definition of the muon selection.}
\label{tab:muonIDs}
\begin{tabular}{c|c|c|c}
\hline
\bf{Selection criteria} & \bf{Loose} & \bf{Fakeable Object} & \bf{Tight} \\
\hline
...
is Medium Muon & -- & \checkmark & \checkmark \\
\hline
\end{tabular}
}
\end{table}
...
Only jets passing loose selection criteria and having \pt > 25\GeV and $|\eta| < 2.4$ are retained.
...
\caption{From left to right: The distributions of \mtmin and \mmin for
  events with an OSSF pair (top) and without an OSSF pair (bottom)} 
...
OSSF pair, events with a pair of loose leptons forming a mass below 5
...
leptons fails the nominal selection but passes the loose requirements
...
selection criteria (refer to Tables~\ref{tab:muonIDs}-~\ref{tab:eleIDs}) but passes loose (\emph{fakeable object}) ones (\emph{application region}), 
and by applying to them  a "tight-to-loose'' ratio or a fake rate (FR
or $f$) which is the probability for 
a nonprompt lepton to pass the tight ID and isolation selection.
...
\noindent where the $\Irel$ is the relative isolation of the lepton calculated in cone with size $\Delta R=0.3$, while the $\Irel^\text{tight}$ is the tight WP defined in tables~\ref{tab:muonIDs} and \ref{tab:eleIDs}.
...
Flavor dependence of the fake rate is important in the estimation of
...
We measure the fake rate maps in data.
...
While measuring the fake rate sin data in lepton+jet events we should take into account the contamination from prompt leptons, mostly from $\PW$ and $\PZ$ production in association with jets.
...
\caption{$\MT$ distributions for muons. In the left column there are
  transverse mass distributions for fakeable objects (denominator) while in the
  right one for tight leptons (numerator). Three rows for the three muon triggers: Mu3, $\ptcone \ \in \
...
\caption{$\MT$ distributions for electron . In the left column there
  are transverse mass distributions for fakeable objects (denominator) while in the
  right one for tight leptons (numerator). Two rows for the two electron triggers:
...
\caption{$\ptcone$ distributions, here labeled $\ptcone$-corrected,
  for muons. In the left column there are $\ptcone$ distributions for fakeable objects (denominator) while in the
  right one for tight leptons (numerator). Three
...
\caption{$\ptcone$ distributions, here labeled $\ptcone$-corrected,
  ...
  for fakeable objects (denominator) while in the
  right one for tight leptons (numerator). Two
...
\paragraph{Fake rate measured in MC}
We also measure the fake rate in MC QCD di-jets and MC $\ttbar$ by
...
the leptons selected with the FO and tight IDs (refer to
Tables~\ref{tab:muonIDs}-~\ref{tab:eleIDs}). Hence the contamination
from prompt leptons in the denominator failing the tight selection is
...
larger than 40\GeV are predicted with fake rate measured with leptons
with \pt $[35-50]$\GeV.
...
\caption{Fake rate values for muons (top) and electron (bottom), calculated in data (red), QCD MC (black) and $\ttbar$ MC(blue).The last bin is not used due to the large EWK contamination, leptons with \pt larger than 40 \GeV are predicted with FR at the bin $[35-50]$\GeV.}
...
\end{itemize}
where $f_{i}$ is the fake rate evaluated on the \ptcone and $\eta$
of the failing lepton.\\
It is essential to point out that for this analysis the softest lepton (trailing) is typically the one the fails the tight selection and it is predicted using the fake rate ratio measured in data. Thus, the fact that the last bin of the FR shown in Fig.~\ref{fig:comparison_muon} is not used does not affect the analysis because there are no fake leptons at high \pt. 


\subsubsection{Validation of the fake rate method in simulation, $\ttbar$ MC }
...
The fake rate extracted from nonprompt leptons in QCD MC events is
...
events with a loose uncleaned b-tagged jet are vetoed. With the above requirements, this control region is extremely pure in ZZ, with other processes being almost negligible.
...
\caption{Comparison of data and MC in the ZZ control region (top) and in the WZ control region (bottom) for several kinematic variables, after normalization of the ZZ/WZ cross section.} 
...
\begin{figure}[h]
\noindent
\makebox[\textwidth]{\includegraphics[width=0.9\textwidth]{Figures/c5/expectedYields/CMS-EXO-17-012_Figure_001.pdf}}
