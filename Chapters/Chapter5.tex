...
The current analysis uses the set of \Pp collision data collected during the 2016 run at a center-of-mass energy of 13\TeV corresponding to
...
\noindent
\makebox[\textwidth]{
\subfloat[]{\includegraphics[width=.33\textwidth]{Figures/c5/genPt/genPtm5.pdf}}
\subfloat[]{\includegraphics[width=.33\textwidth]{Figures/c5/genPt/genPtm60.pdf}}
\subfloat[]{\includegraphics[width=.33\textwidth]{Figures/c5/genPt/genPtm200.pdf}}}
...
still be seen to prefer relatively low momenta because of the high off-shell W-boson required for the production of very heavy
HNL's. Furthermore, the cross-section for off-shell W-boson production drops
rapidly with increasing mass, and as such the lepton with the smallest \pt
...
The triggers used in this analysis are listed in table~\ref{table:HNL_trigger_thresholds}.
...
\item Each trigger path could select one or more leptons (refer to
  ...
  will be referred to as lepton-specific efficiency. Then data/MC scale
...
from fakes at the sub-percent level. We, therefore, do not perform any background subtraction using a fit to 
...
lepton \pt in both the unbiased \verb!MET! dataset, a high-statistics $\PW\PZ$ MC sample, and a HNL sample. 
The applied baseline selection criteria
ensure we are on the efficiency plateau, see Figure~\ref{fig:3l1l1lEff}, and reasonable data/MC agreement is observed 
as statistical uncertainties of the samples allow us to judge.}
\fi
...
is the \pt thresholds that are being used. Considering the earlier
...
leptons; events with an OSSF (Opposite Sign Same Flavor) pair and
...
As the name suggests the low mass region focuses on
...
internal conversions (a detailed explanation of this background
contribution is given in Section~\ref{sec:conversion}). In such events, a lepton might radiate a real or
...
events is expected from the Z + $\gamma$ process, mainly having \mlll
...
anticipated earlier. The first category consists of events with leading
...
leading lepton \pt values below 30\GeV and is mainly used for inquiring
about the existence of HNLs with masses close to that of the W boson. In
...
\mtmin and \mmin, a very large number of search regions that were
...
\label{fig:willem3L}
\end{wrapfigure}
 selection criteria are measured in a multi-jet sample in data enriched
...
calculated in a cone with size $\Delta R=0.3$, while the
...
\PZ production in association with a jet is located in the region with
...
with \pt $[35-50]$\GeV.\\
It is essential to point out that for this analysis the softest lepton (trailing) is typically the one that fails the \ti selection and it is predicted using the \fr ratio measured in data. Thus, the fact that the last bin of the FR shown in Figure~\ref{fig:comparison_muon} is not used does not affect the analysis because there are no fake leptons at high \pt. 
...
To measure the normalization factors, a simultaneous fit of the three
...
control regions and was found negligible considering as \mixpar the
...
detector material (external conversion) constitute one of the main backgrounds in the
 ...
 regions with OSSF pairs by removing events where an OSSF pair has a mass below 5\GeV.

In order to normalize and validate this background, we use a control
region defined requiring the presence of an OSSF pair and \mlll to be compatible with the Z mass within a 15\GeV
...
and associated uncertainty in this correction is derived to be~1--5\%~\cite{btagSF}. 
...
\subsection{Interpretation}
 ...
 interpolation between the mass points and assuming only coupling to either $\nu_{\mu}$ or $\nu_{\Pe}$,
...
\begin{figure}[h]
\centering
\includegraphics[width=.70\textwidth]{Figures/c5/limits/CMS-EXO-17-012_Figure_002-a.pdf}\\
\includegraphics[width=.70\textwidth]{Figures/c5/limits/CMS-EXO-17-012_Figure_002-b.pdf}
...
tau objects. To give the reference point, the \emph{singleTau} trigger has
